...
#ifndef PRODUCT
void g1NUMASpace::mangle_unused_area() {
  // This method should do nothing.
  // It can be called on a numa space during a full compaction.
}
void g1NUMASpace::mangle_unused_area_complete() {
  // This method should do nothing.
  // It can be called on a numa space during a full compaction.
}
void g1NUMASpace::mangle_region(MemRegion mr) {
  // This method should do nothing because numa spaces are not mangled.
}
void g1NUMASpace::set_top_for_allocations(HeapWord* v) {
  assert(false, "Do not mangle g1NUMASpace's");
}
void g1NUMASpace::set_top_for_allocations() {
  // This method should do nothing.
}
void g1NUMASpace::check_mangled_unused_area(HeapWord* limit) {
  // This method should do nothing.
}
void g1NUMASpace::check_mangled_unused_area_complete() {
  // This method should do nothing.
}
#endif  // NOT_PRODUCT
...
void g1NUMASpace::ensure_parsability() {
  for (int i = 0; i < lgrp_spaces()->length(); i++) {
    LGRPSpace *ls = lgrp_spaces()->at(i);
    MutableSpace *s = ls->space();
...
size_t g1NUMASpace::used_in_words() const {
  size_t s = 0;
  for (int i = 0; i < lgrp_spaces()->length(); i++) {
...
size_t g1NUMASpace::free_in_words() const {
  size_t s = 0;
  for (int i = 0; i < lgrp_spaces()->length(); i++) {
...
size_t g1NUMASpace::tlab_capacity(Thread *thr) const {
...
size_t g1NUMASpace::tlab_used(Thread *thr) const {
...
size_t g1NUMASpace::unsafe_max_tlab_alloc(Thread *thr) const {
  // Please see the comments for tlab_capacity().
  guarantee(thr != NULL, "No thread");
  int lgrp_id = thr->lgrp_id();
  if (lgrp_id == -1) {
...
size_t g1NUMASpace::capacity_in_words(Thread* thr) const {
  guarantee(thr != NULL, "No thread");
  int lgrp_id = thr->lgrp_id();
  if (lgrp_id == -1) {
    if (lgrp_spaces()->length() > 0) {
...
bool g1NUMASpace::update_layout(bool force) {
...
void g1NUMASpace::scan_pages(size_t page_count)
{
...
void g1NUMASpace::accumulate_statistics() {
  if (UseAdaptiveNUMAChunkSizing) {
    for (int i = 0; i < lgrp_spaces()->length(); i++) {
...
// chunk resizing.
size_t g1NUMASpace::current_chunk_size(int i) {
...
// page_size() aligned.
size_t g1NUMASpace::default_chunk_size() {
...
// lgrp_spaces()->length().
size_t g1NUMASpace::adaptive_chunk_size(int i, size_t limit) {
...
// |------------------new_region---------------------------------|
// |----bottom_region--|---intersection---|------top_region------|
void g1NUMASpace::select_tails(MemRegion new_region, MemRegion intersection,
...
// |----------------|-------invalid---|--------------------------|
// |----bottom_region--|---intersection---|------top_region------|
void g1NUMASpace::merge_regions(MemRegion new_region, MemRegion* intersection,
...
void g1NUMASpace::set_top(HeapWord* value) {
...
void g1NUMASpace::clear(bool mangle_space) {
...
HeapWord* g1NUMASpace::allocate(size_t size) {
...
HeapWord* g1NUMASpace::cas_allocate(size_t size) {
  Thread* thr = Thread::current();
  int lgrp_id = thr->lgrp_id();
  if (lgrp_id == -1 || !os::numa_has_group_homing()) {
    lgrp_id = os::numa_get_group_id();
...
void g1NUMASpace::print_short_on(outputStream* st) const {
...
void g1NUMASpace::print_on(outputStream* st) const {
...
void g1NUMASpace::LGRPSpace::accumulate_statistics(size_t page_size) {
  clear_space_stats();
...
void g1NUMASpace::LGRPSpace::scan_pages(size_t page_size, size_t page_count)
{
